(function() {
    ...
    TableRenderer.prototype = {
        ...
        renderFromNode: function(table) {
            ...
            if (table && (data = this.getTableNodeData(table))) {
                result = this.renderCsv(data);
            }
            return result;
        },
        getTableNodeData: function(table) {
            var result = [],
                trs, tds, td, text, i, il, j, jl, row;
            if (null == table) {
            ...
            for (i = 0, il = trs.length; i < il; ++i) {
                row = [];
                if (trs[i].firstChild.tagName === 'TH') {
                    if (i > 1) {
                        console.log('continue', trs[i]);
                        continue;
                    }
                    tds = trs[i].querySelectorAll('th');
                } else {
    ...
    var text = (new TableRenderer()).renderFromNode(table),
    ...
    if (wnd) {
        ...
        wnd.opener = self;
        wnd.focus();
        wnd.document.write('<textarea style="width:100%; height:100%;">' + html + '</textarea>');
        wnd.stop();
    }
})();